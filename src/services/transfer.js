...
module.exports = (app) => {
  ...
  const validate = async (transfer) => {
    ...
    if (!transfer.acc_origin_id) throw new ValidationError('Conta de origem é um atributo obrigatório');
    if (!transfer.acc_dest_id) throw new ValidationError('Conta de destino é um atributo obrigatório');
    if (transfer.acc_origin_id === transfer.acc_dest_id) throw new ValidationError('Não é possivel transferir de uma conta para ela mesma');

    const accounts = await app.db('accounts').whereIn('id', [transfer.acc_dest_id, transfer.acc_origin_id]);

    accounts.forEach(acc => {
      if (acc.user_id !== parseInt(transfer.user_id, 10)) throw new ValidationError(`A conta #${acc.id} não pertence ao usuário`);
    });
  };

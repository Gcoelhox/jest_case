...
beforeAll(async () => {
  ...
  const users = await app.db('users').insert([
    { name: 'User #1', mail: 'usermail@mail.com', passwd: '$2a$10$RDBRng/QxRUzUihLtVhGE.i4K.4NbC2QldDnkuKVv4Bojul3P/04i' },
    { name: 'User #2', mail: 'user2mail@mail.com', passwd: '$2a$10$RDBRng/QxRUzUihLtVhGE.i4K.4NbC2QldDnkuKVv4Bojul3P/04i' },
  ], '*');
  [user, user2] = users;
  delete user.passwd;
...
test('Deve inserir uma transação com sucesso', () => {
  return request(app).post(MAIN_ROUTE)
    .set('authorization', `Bearer ${user.token}`)
    .send({
      description: 'New T', date: new Date(), ammount: 150, type: 'I', acc_id: accUser.id,
    })
    .then(res => {
...
test('Deve retornar uma transação por ID', () => {
  return app.db('transactions').insert({
    description: 'T Id', date: new Date(), ammount: 150, type: 'I', acc_id: accUser.id,
...
test('Deve alterar uma transação', () => {
  return app.db('transactions').insert({
    description: 'to Update', date: new Date(), ammount: 150, type: 'I', acc_id: accUser.id,
...
test('Não deve remover uma transação de outro usuário', () => {
  return app.db('transactions').insert({
    description: 'to Update', date: new Date(), ammount: 150, type: 'I', acc_id: accUser2.id,
  }, ['id']).then(trans => request(app).delete(`${MAIN_ROUTE}/${trans[0].id}`)
    .set('authorization', `Bearer ${user.token}`)
    .then(res => {
...
test('Não deve remover conta com transação', () => {
  return app.db('transactions').insert({
    description: 'to Delete', date: new Date(), ammount: 150, type: 'I', acc_id: accUser.id,
...
test('Transações de saída devem ser negativas', () => {
  return request(app).post(MAIN_ROUTE)
    .set('authorization', `Bearer ${user.token}`)
    .send({
      description: 'New T', date: new Date(), ammount: 150, type: 'O', acc_id: accUser.id,
    })
    .then(res => {
      expect(res.status).toBe(201);
      expect(res.body.acc_id).toBe(accUser.id);
...
describe('Ao tentar inserir uma transação válida', () => {
  ...
  test('Não deve inserir uma transação com tipo inválido', () => testTemplate({ type: 'P' }, 'Tipo inválido'));
});
